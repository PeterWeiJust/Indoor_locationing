...
class SensorDataset(torch.utils.data.Dataset):
    def __init__(self,mode="train",transform=torch.from_numpy):
        ...
        self.testx,self.testy=read_data(7,7)
...
class WifiDataset(torch.utils.data.Dataset):
    def __init__(self,mode="train",transform=torch.from_numpy):
        ...
        self.testx,self.testy=read_wifi_data(7,7)
        self.length=len(self.trainx)+len(self.testx)
...
class DownsampleDataset(torch.utils.data.Dataset):
    def __init__(self,tw=1000,slide=100,mode="train",transform=torch.from_numpy):
        ...
        self.sensorval,self.labelval,self.wifival=downsample_data(1,1,tw,slide)
        for i in range(9,8):
        ...
        self.sensortest,self.labeltest,self.wifitest=downsample_data(7,7,tw,slide)
...
def read_wifi_data(file_start,file_end):
    if file_start==file_end:     
        path='sensordata/sensor_wifi_5_'+str(file_start)+'_timestep100.csv'
        ...
        for file_num in range (file_start-1,file_end):
            file_num=file_num+1
            path='sensordata/sensor_wifi_5_'+str(file_num)+'_timestep100.csv'
...
def read_data(file_start,file_end):
    if file_start==file_end:
        ...
        path='timestep100/5_'+str(file_start)+'timestep100.csv'
        label_path='sensordata/sensor_wifi_5_'+str(file_start)+'_timestep100.csv'
        ...
        for file_num in range (file_start-1,file_end):
            ...
            path='timestep100/5_'+str(file_num)+'timestep100.csv'
            label_path='sensordata/sensor_wifi_5_'+str(file_num)+'_timestep100.csv'
...
def count_label_num(file_num):
    countlabel_path='sensordata/sensor_wifi_5_'+str(file_num)+'_timestep100.csv'
