# -*- coding: utf-8 -*-
"""
Created on Sat Jun  6 11:04:16 2020
...
model_name = "wifiSVM_classification_bucharest_training"
...
# Fit model
model = SVC(kernel='rbf', random_state=0, gamma="auto", C=1)
model.fit(WifiTrain, locationlabel)
...
fig1=plt.figure()
locPrediction = model.predict(WifiTrain)
#locpredlabel=np.argmax(locPrediction,axis=1)
locpredlabel=locPrediction
...
loctrainlatlng=pf.convert_data_to_utm(locationlabel)
data=pf.normalized_data(np.hstack((loctrainlatlng,loclatlng)))
...
#draw cdf picture
fig=plt.figure()
bin_edge,cdf=pf.cdfdiff(target=loctrainlatlng,predict=loclatlng)
